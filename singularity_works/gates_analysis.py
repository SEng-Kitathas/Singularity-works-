...
from .gates_types import GateFinding
...
def _deep_nesting_finding(deep_nesting: int):
    ...
    suggestion = SimplificationSuggestion(
        ...
        rationale="Deep nesting reduces auditability and increases error surface",
        expected_gain="medium",
        confidence="moderate",
        rewrite_candidate="# introduce guard clauses or extract nested block into helper",
    )
    return GateFinding(
        "deep_nesting",
        f"Control-flow nesting depth {deep_nesting} exceeds review threshold",
        "medium",
...
def _abstraction_pressure_finding(function_count: int, line_count: int, declarative_module: bool):
    if declarative_module or function_count != 1 or line_count <= 140:
        return None
    suggestion = SimplificationSuggestion(
        suggestion_id="suggest:split_single_function",
        summary="Split oversized single-function artifact",
        rationale="Large single-function artifacts hide multiple responsibilities",
        expected_gain="medium",
        confidence="moderate",
        rewrite_candidate="# extract coherent sub-steps into named helpers",
    )
    return GateFinding(
