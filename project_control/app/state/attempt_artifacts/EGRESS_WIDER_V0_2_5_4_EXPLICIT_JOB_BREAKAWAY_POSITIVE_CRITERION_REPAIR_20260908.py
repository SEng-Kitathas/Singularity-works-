...
"""Frozen wider-egress v0.2.5.4: explicit CREATE_BREAKAWAY_FROM_JOB positive-criterion repair.
...
requires that outer Job to expose SILENT_BREAKAWAY_OK. The helper/root and returned
positive child may still belong to some outer Job; any-Job membership is retained as
telemetry and is not treated as membership in the Forge immediate Job.
...
v0.2.5.4 repairs only the over-strong positive-control acceptance criterion after
immutable v0.2.5.3. It binds control 2726d4ed... / CHECKPOINT 6378560c... and
runSync plane record de4e66d5.... The helper source/DLL and exact CreateProcessW
flags are unchanged. Positive explicit creation + child survival after root exit is
sufficient; any-Job membership is telemetry only. Cleanup, outer-Job gate, protected
primitive, protected survival test, non-network scope and claim ceiling are unchanged.
"""
...
EXPECTED_CONTROL_HEAD = "2726d4edbfa2d1af3977b4dda81c26da9385320d"
EXPECTED_CONTROL_CHECKPOINT = "6378560c5ed9993a302fc88386cb47d789cdc01f8895fa99e460dbc0b90bd264"
GEN14_CHECKPOINT = "checkpoint-app-live-0014-43aa7feac7e8"
PARENT_ATTEMPT_ID = "attempt-egress-wider-v0-2-5-4-synchronous-execution-plane-currentness-post-2726d4ed"
ATTEMPT_ID = "attempt-egress-wider-v0-2-5-4-explicit-job-breakaway-positive-criterion-repair-first"
...
class ProcessProbe:
    ...
    def job_membership(self, pid: int) -> dict[str, object]:
        ctypes.set_last_error(0)
        handle = self.kernel32.OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION | SYNCHRONIZE, False, pid)
        if not handle:
            return {"pid": pid, "queried": False, "in_any_job": None, "open_error": ctypes.get_last_error()}
        try:
            in_job = wintypes.BOOL()
            ctypes.set_last_error(0)
            ok = bool(self.kernel32.IsProcessInJob(handle, None, ctypes.byref(in_job)))
            return {
                "pid": pid,
                "queried": ok,
                "in_any_job": bool(in_job.value) if ok else None,
                "query_error": 0 if ok else ctypes.get_last_error(),
            }
        finally:
            self.kernel32.CloseHandle(handle)
...
def _emit(result: dict[str, object], exit_code: int) -> int:
    result["schema"] = "singularity-works.egress-wider-v0.2.5.4-explicit-job-breakaway-result/0.1"
...
def main() -> int:
    ...
    try:
        ...
        if positive_decoded.get("kind") not in {"BREAKAWAY_CREATE_SUCCESS_NOT_IN_JOB", "BREAKAWAY_CREATE_SUCCESS_IN_JOB"}:
            result["reason"] = "positive-control helper did not prove explicit breakaway child creation success"
            return _emit(result, 3)
        positive_child = int(positive_decoded["pid"])
        positive_state = probe.state(positive_child)
        positive_membership = probe.job_membership(positive_child)
        result["positive_control"]["child_state_after_root_exit"] = positive_state
        result["positive_control"]["child_job_membership_after_root_exit"] = positive_membership
        if positive_state.get("alive") is not True:
            result["reason"] = "positive-control explicit-breakaway child was not alive after root exit"
            return _emit(result, 3)
        ...
        if kind in {"BREAKAWAY_CREATE_SUCCESS_IN_JOB", "BREAKAWAY_CREATE_SUCCESS_NOT_IN_JOB"}:
            child_pid = int(decoded["pid"])
            state = probe.state(child_pid)
            membership = probe.job_membership(child_pid)
            result["protected"]["breakaway_child_state_after_return"] = state
            result["protected"]["breakaway_child_job_membership_after_return"] = membership
            ...
            result["reason"] = "explicit breakaway creation reported success, but returned child was not alive after protected Job close"
            return _emit(result, 3)
