...
"""Frozen wider-egress v0.2.5.3: explicit CREATE_BREAKAWAY_FROM_JOB child-membership repair.
...
v0.2.5.3 repairs only the Python observation layer after immutable v0.2.5.2.
It binds verified control 8071b094... / CHECKPOINT 10021107... and direct runSync
plane record df57157d.... The helper source/DLL and exact CreateProcessW flags are
unchanged. The repair observes returned explicit-child PID liveness and any-Job
membership, rather than inferring child membership from the helper/root's Job state.
Outer-Job gate, normal-child baseline, cleanup, protected primitive, non-network
...
EXPECTED_CONTROL_HEAD = "8071b0945ee7f9a7709eef0ef62c42141a2ebc39"
EXPECTED_CONTROL_CHECKPOINT = "1002110708b3bd53db376249dd384cfe26dcfb2d6aa6340880124f8cea6cb021"
GEN14_CHECKPOINT = "checkpoint-app-live-0014-43aa7feac7e8"
PARENT_ATTEMPT_ID = "attempt-egress-wider-v0-2-5-3-synchronous-execution-plane-currentness-post-8071b094"
ATTEMPT_ID = "attempt-egress-wider-v0-2-5-3-explicit-job-breakaway-child-membership-repair-first"
...
def _emit(result: dict[str, object], exit_code: int) -> int:
    result["schema"] = "singularity-works.egress-wider-v0.2.5.3-explicit-job-breakaway-result/0.1"
...
def main() -> int:
    ...
    try:
        ...
        if positive_state.get("alive") is not True:
            ...
            return _emit(result, 3)
        if positive_membership.get("queried") is not True or positive_membership.get("in_any_job") is not False:
            result["reason"] = "positive-control explicit-breakaway child was not verified outside all Jobs"
            return _emit(result, 3)
