...
"""Frozen wider-egress v0.2.7.2: WMI Win32_Process.Create HRESULT-encoder repair.
...
v0.2.7.2 repairs only the two frozen WMI exception encoders after immutable
v0.2.7.1: negative HRESULTs are masked directly with `-band 0xFFFF` instead of
being converted to `[uint32]` first. Stage prefixes remain 0x2... for class bind and
0x3... for Create invocation. Control/lineage/schema bindings update to the
fresh-clone-verified v0.2.7.1 harness checkpoint. Positive WMI Create, child command,
success/ReturnValue encodings, lineage/liveness/cleanup, protected primitive,
non-network scope and claim ceiling remain unchanged.
...
EXPECTED_CONTROL_HEAD = "dbe988b4a6773a79d31935b46984b17fb9c4c421"
EXPECTED_CONTROL_CHECKPOINT = "f84f5b8886dd56c7c9ec8baa52c34f5c08a3e1890074f76188d9e110a58e6f67"
GEN14_CHECKPOINT = "checkpoint-app-live-0014-43aa7feac7e8"
PARENT_ATTEMPT_ID = "attempt-egress-wider-v0-2-7-1-wmi-harness-invalid-diagnosis"
ATTEMPT_ID = "attempt-egress-wider-v0-2-7-2-wmi-win32-process-create-hresult-encoder-repair-first"
...
def _root_command(powershell: Path) -> list[str]:
    ...
    script = (
        ...
        "catch { $low=($_.Exception.HResult -band 0xFFFF); exit [int](0x20000000 -bor $low) }; "
        f"$cmd='{child}'; "
        "try { $r=$c.Create($cmd) } "
        "catch { $low=($_.Exception.HResult -band 0xFFFF); exit [int](0x30000000 -bor $low) }; "
...
def _emit(result: dict[str, object], exit_code: int) -> int:
    result["schema"] = "singularity-works.egress-wider-v0.2.7.2-wmi-win32-process-create-result/0.1"
