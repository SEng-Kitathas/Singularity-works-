...
from forge_semantic_delta_replay_v0_1 import build_plan, replay_plan, SemanticDeltaPlan, DeltaOperation
from forge_semantic_fact_ir_v0_1_2 import canonical_json

TARGET = ROOT / "investigation" / "vuln_benchmark_quarry_20260902" / "PyGoat"
RUN = ROOT / "investigation" / "sw_forge_reconstruction" / "semantic_delta_replay_pygoat_v0_1_20260909"
...
def main() -> int:
    ...
    summary={
        "schema":"forge.semantic-delta-replay-pygoat/0.1",
        ...
        "artifact_hashes":{
            "replay":hashlib.sha256((CODE/"forge_semantic_delta_replay_v0_1.py").read_bytes()).hexdigest(),
    ...
    (RUN/"output_manifest.json").write_text(json.dumps({"schema":"forge.semantic-delta-replay-output-manifest/0.1","files":manifest,"count":len(manifest)},indent=2,sort_keys=True)+"\n",encoding="utf-8")
