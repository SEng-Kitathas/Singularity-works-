...
v0.1.1 repairs only frozen-dataclass serialization: `dataclasses.asdict()` is
replaced by a shallow field extractor, leaving nested MappingProxy-backed values for
the existing canonical JSON normalizer to thaw without deepcopy. Algebra/replay
semantics are unchanged.


v0.1.2 additionally repairs only the mutable replay workspace boundary: frozen
MappingProxy-backed nested values are recursively thawed into ordinary build-bundle
containers before replay. Operation grammar and acceptance criteria are unchanged.


v0.1.3 repairs only UPSERT insertion: after the frozen payload digest is verified,
the target dataclass is reconstructed as a mutable-safe collection value before
insertion. Plan construction, operation identity/order and replay gates are unchanged.
...
def _record(value: Any) -> dict[str, Any]:
    if not is_dataclass(value):
        raise TypeError(f"semantic delta object is not a dataclass: {type(value).__name__}")
    return {f.name: getattr(value, f.name) for f in fields(value)}


def _digest(value: Any) -> str:
    return hashlib.sha256(canonical_json(_record(value)).encode("utf-8")).hexdigest()
...
class DeltaOperation:
    ...
    def descriptor(self) -> dict[str, Any]:
        return {
            ...
            "value": _record(self.value) if self.value is not None else None,
        }
...
def _copy_bundle(base: FrozenSemanticFactBundle) -> SemanticFactBundle:
    return SemanticFactBundle(
        ...
        entities={
            k: SemanticEntity(v.entity_id, v.kind, v.name, v.source_id, v.evidence_id, _thaw(v.properties))
            for k, v in base.entities.items()
        },
        facts={
            k: SemanticFact(v.fact_id, v.subject_id, v.predicate, _thaw(v.object_value), tuple(v.evidence_ids),
                            v.evidence_status, v.producer, v.assurance_ceiling, tuple(v.derivation_from), _thaw(v.properties))
            for k, v in base.facts.items()
        },
        unknowns={
            k: UnknownSeam(v.seam_id, v.subject_id, v.question, v.reason, tuple(v.evidence_ids), v.producer,
                           v.assurance_ceiling, v.blocking, _thaw(v.properties))
            for k, v in base.unknowns.items()
        },
...
def _mutable_operation_value(collection: str, value: Any) -> Any:
    if collection == "sources":
        if not isinstance(value, SourceReferent):
            raise TypeError("source UPSERT payload type mismatch")
        return value
    if collection == "evidence":
        if not isinstance(value, EvidenceSpan):
            raise TypeError("evidence UPSERT payload type mismatch")
        return value
    if collection == "entities":
        if not isinstance(value, SemanticEntity):
            raise TypeError("entity UPSERT payload type mismatch")
        return SemanticEntity(value.entity_id, value.kind, value.name, value.source_id, value.evidence_id, _thaw(value.properties))
    if collection == "facts":
        if not isinstance(value, SemanticFact):
            raise TypeError("fact UPSERT payload type mismatch")
        return SemanticFact(value.fact_id, value.subject_id, value.predicate, _thaw(value.object_value), tuple(value.evidence_ids),
                            value.evidence_status, value.producer, value.assurance_ceiling, tuple(value.derivation_from), _thaw(value.properties))
    if collection == "unknowns":
        if not isinstance(value, UnknownSeam):
            raise TypeError("unknown UPSERT payload type mismatch")
        return UnknownSeam(value.seam_id, value.subject_id, value.question, value.reason, tuple(value.evidence_ids), value.producer,
                           value.assurance_ceiling, value.blocking, _thaw(value.properties))
    raise ValueError(f"unsupported collection: {collection}")
    ...
    *,
    ...
    for op in plan.operations:
        ...
        if op.action == "REMOVE":
            ...
            coll[op.key] = _mutable_operation_value(op.collection, op.value)
        else:
